...
export default function Navbar({ onMenuClick }: Props) {
  ...
  const save = async () => {
    ...
    if (mode === "add")    await addAddresses(text);
    setText(""); setMode(null);
  };

  return (
    <header className="bg-green-600 text-white h-12 flex items-center px-3">
      <button onClick={onMenuClick} className="text-2xl mr-3" aria-label="menu">≡</button>
      <h1 className="font-bold mr-auto select-none">GrassGPS</h1>

      <button className="btn bg-white text-green-700"    onClick={()=>setMode("import")}>Import List</button>
      <button className="btn bg-white text-green-700 ml-2" onClick={()=>setMode("add")}>Add Addr</button>
      ...
      {mode && (
        <Modal onClose={()=>setMode(null)}>
          ...
          />
          <button className="btn w-full" onClick={save} disabled={!text.trim()}>Save</button>
        </Modal>
      )}
    </header>
  );
